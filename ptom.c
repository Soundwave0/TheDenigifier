...
static const char s_token[134][25]= {
    "", "function ", "function ", "if", "switch", "try", "while", "for", "end", "else",
    "elseif", "break", "return ", "parfor", "", "global ", "persistent ", "", "", "",
    "catch ", "continue ", "case ", "otherwise", "", "classdef ", "", "", "properties ", "",
    "methods ", "events ", "enumeration ", "spmd ", "parsection ", "section ", "", "", "", "",
    "id ", "end", "int ", "float ", "string ", "dual ", "bang ", "?", "", "", "; ", ",",
    "(", ")", "[", "]", "{", "}", "feend ", "", "' ", "dottrans ", "~", "@", "$", "`", "\"",
    "", "", "", "+", "-", "*", "/", "\\", "^", ":", "", "", "", ".", ".*", "./", ".\\", ".^",
    "&", "|", "&&", "||", "<", ">", "<=", ">=", "==", "~=", "=", "cne ", "arrow ", "", "",
    "\n", "\n ", "\n ", "...\n ", "", "comment ", "blkstart ", "blkcom ", "blkend ", "cpad ",
    "pragma ", "...", "..", "deep_nest ", "deep_stmt ", "", "white ", "", "negerr ", "semerr ",
    "eolerr ", "unterm ", "badchar ", "deep_paren ", "fp_err ", "res_err ", "deep_com ",
    "begin_type ", "end_type ", "string_literal ", "unterm_string_literal ", "arguments_block ",
    "last_token ", ""
};

// FIXED: Auto-sized arrays to prevent buffer overflow
static const char s_major_version[] = "v01.00";
static const char s_minor_version[] = "v00.00";
...
static int s_uncompress(struct mfile_t *mfile, struct pfile_t *pfile)
{
    ...
    if (uncompress(tmp_ptr, &size, pfile->pdata, (uLongf)pfile->size_after_compass) != Z_OK)
    {
        free(tmp_ptr);
        return 0;
    }
    ...
    size -= 7 * sizeof(u32);
    mfile->mdata = (u8*)malloc(size);  // FIXED: Cast to u8*
    if (mfile->mdata)
    {
...
static int s_parsePFile(struct pfile_t *pfile, char *path)
{
    ...
    if (pfp)
    {
        ...
        if (psize >= 32)
        {
            ...
            {
                pfile->pdata = (u8*)malloc(pfile->size_after_compass);  // FIXED: Cast to u8*
                if (pfile->pdata)
                {
...
static int s_parseMFile(char* mpath, struct mfile_t *mfile)
{
    ...
    if (success)
    {
        ...
        if (fp)
        {
            fwrite(mfile_ptr, 1, mfile_tmp - mfile_ptr, fp);
            fclose(fp);
        }
    }
